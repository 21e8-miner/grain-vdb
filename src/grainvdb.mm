/**
 * GrainVDB Native Implementation
 * -----------------------------
...
// Half-precision conversion (IEEE 754-2008)
...
struct gv1_state_t {
  ...
  uint32_t rnk;
  id<MTLBuffer> m_buf; // Primary Manifold Buffer (Half precision)
  uint32_t e_cnt;      // Entry count
  id<MTLLibrary> lib;
};
...
gv1_state_t *gv1_ctx_create(uint32_t rank, const char *library_path) {
  ...
  if (!state->lib) {
    std::cerr << "GrainVDB Native Error: Could not find " << library_path
              << std::endl;
    delete state;
    return nullptr;
  }
...
void gv1_data_feed(gv1_state_t *state, const float *buffer, uint32_t count) {
  ...
  uint32_t byte_length = total_elements * sizeof(gv_half_t);

  // Allocate Shared (Unified) Memory
  state->m_buf = [state->dev newBufferWithLength:byte_length
...
float gv1_manifold_fold(gv1_state_t *state, const float *probe, uint32_t top,
  ...
  // 2. Prepare Result Scores Buffer
  id<MTLBuffer> s_buf =
  ...
  NSUInteger max_threads = state->p_state.maxTotalThreadsPerThreadgroup;
  MTLSize t_sz =
      MTLSizeMake(std::min((uint32_t)max_threads, state->e_cnt), 1, 1);
  ...
  // 4. CPU-Side Top-k Selection (Priority Queue)
  float *scores = (float *)[s_buf contents];
  typedef std::pair<float, uint64_t> ResultPair;
  ...
  // Return duration in explicit MILLISECONDS
...
float gv1_topology_audit(gv1_state_t *state, const uint64_t *map,
  ...
  // Extract results into local float memory for analysis
  std::vector<float> neighbors(count * state->rnk);
  for (uint32_t i = 0; i < count; i++) {
    uint64_t offset = map[i] * state->rnk;
    for (uint32_t k = 0; k < state->rnk; k++) {
      neighbors[i * state->rnk + k] = f16_to_f32(m_ptr[offset + k]);
    }
  }

  // Thresholded Connectivity Check (Semantic Neighborhood Density)
